...
export function Room() {
    ...
    async function handleSendQueston(event: FormEvent) {
        ...
        await set(insertQuestion, question).then(() => {console.log('Sucesso')});
    ...
    async function handleLikeQuestion(questionId: string, likeId: string | undefined){
        if (likeId) {
            const like =  ref(getDatabase(), `rooms/${roomId}/questions/${questionId}/likes/${likeId}`);
            await remove(like);
        } else {
            const like =  ref(getDatabase(), `rooms/${roomId}/questions/${questionId}/likes`);
            const newLike = push(like);
            await set(newLike, {authorId: user?.id});
        }
    }
